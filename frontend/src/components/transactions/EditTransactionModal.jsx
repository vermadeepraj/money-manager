...
export function EditTransactionModal() {
    ...
    const isOpen = activeModal === MODALS.EDIT_TRANSACTION
    ...
    useEffect(() => {
        if (isOpen && transaction) {
            ...
            Promise.all([
                ...
                setValue('categoryId', transaction.categoryId?._id || transaction.categoryId || '')
                setValue('accountId', transaction.accountId?._id || transaction.accountId || '')
                setValue('description', transaction.description || '')
                setValue('date', new Date(transaction.date).toISOString().split('T')[0])
                setValue('division', transaction.division || DIVISIONS.PERSONAL)
            }).catch(console.error)
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} size="lg">
            <ModalContent>
                ...
                <ModalBody>
                    <form id="edit-transaction-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                        {/* Type Toggle */}
